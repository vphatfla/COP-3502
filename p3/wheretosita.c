...
int countNumberOfProperPermutation;
...
int main()
{
    ...
    printf("%d\n", countNumberOfPermuatation);
    // free the memory
    for (int i = 0; i < numberOfPeople; i += 1)
    {
        // printf("%p vs %p\n", (void *)listPeople[i]->listPeopleNotSitNext, (void *)listPeople[i]);
        // fflush(stdout);
...
void permute(person **listPeople, int start, int end)
{
    if (start == end)
    {
        ...
        // for (int i = 0; i < numberOfPeople; i += 1)
        // {
        //     printf("%s   ---   ", listPeople[i]->name);
        // }

        if (checkPermuatedSeating(listPeople) == 1)
        {
            countNumberOfPermuatation += 1;
            // for (int i = 0; i < numberOfPeople; i += 1)
            // {
            //     printf("%s   ---   ", listPeople[i]->name);
            // }
